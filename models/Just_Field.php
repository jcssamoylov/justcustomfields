...
class Just_Field{
	 ...
	 *	Constructor
	 */
	public function __construct( $data_layer, $post_type = NULL, $id_base, $title, $field_options = array() ){
	...
	public function set_id( $id ){
		...
		// parse number
			...
			// load instance data
			$this->instance =(array)$this->_dataLayer->get_fields( $this->post_type, $this->id );
	...
	public function get_field_id( $str, $delimeter = '-' ){
		...
		if( $this->is_collection_field() && $this->is_post_edit ){
			$collection = $field_factory->initObject($this->post_type, $this->collection_id, $this->fieldset_id);
			return str_replace('-',$delimeter,'field'.$delimeter.$collection->id_base.$delimeter.$collection->number.$delimeter
					.\JCF\components\collection\Just_Field_Collection::$current_collection_field_key.$delimeter.$this->id.$delimeter.$str);
		}
	...
	public function get_field_name( $str ){
		...
		if( $this->is_collection_field() && $this->is_post_edit ){
			$collection = $field_factory->initObject($this->post_type, $this->collection_id, $this->fieldset_id);
			return 'field-'.$collection->id_base.'['.$collection->number.']['. \JCF\components\collection\Just_Field_Collection::$current_collection_field_key.']['.$this->id.']['.$str.']';
		}
						...
						<?php if($this->is_collection_field()) : ?>
							...
							<p>
								...
								<select class="widefat" 
									...
									<?php foreach( \JCF\components\collection\Just_Field_Collection::$field_width as $key => $width) : ?>
	 ...
	 */
	public function do_update($field_index, $params = array()){
		...
		if( !$this->is_collection_field() ){
			// update fieldset
			$fieldset = $this->_dataLayer->get_fieldsets( $this->post_type, $this->fieldset_id );
			$fieldset['fields'][$this->id] = $instance['enabled']; 
			$this->_dataLayer->update_fieldsets( $this->post_type, $this->fieldset_id, $fieldset );
			...
			// save
			$this->_dataLayer->update_fields($this->post_type, $this->id, $instance, $this->fieldset_id);
			...
			// save
			$this->_dataLayer->update_fields($this->post_type, $this->collection_id, $collection->instance, $this->fieldset_id);
	...
	public function do_delete(){
		...
		$this->_dataLayer->update_fields($this->post_type, $this->id, NULL, $this->fieldset_id);
	}
