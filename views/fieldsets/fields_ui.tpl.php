	...
	<div class="jcf_columns jcf_width66p">
		...
		if( !empty($fieldsets) ) : ?>
			...
			<div class="jcf_inner_box" id="jcf_fieldset_<?php echo $fieldset['id']; ?>">
				<h3 class="header"><span class="drag-handle">move</span><?php _e('Fieldset:', \jcf\JustCustomFields::TEXTDOMAIN); ?> <span><?php echo $fieldset['title']; ?></span>
					<small>
						<a href="#" class="jcf_fieldset_change jcf_change_pt" rel="<?php echo $fieldset['id']; ?>"><?php _e('change', \jcf\JustCustomFields::TEXTDOMAIN); ?></a>
						<a href="#" class="jcf_fieldset_delete jcf_change_pt" rel="<?php echo $fieldset['id']; ?>"><?php _e('delete', \jcf\JustCustomFields::TEXTDOMAIN); ?></a>
					</small>
					...
					<table class="wp-list-table widefat fixed fieldset-fields-table" cellspacing="0">
						...
						<tbody id="the-list-<?php echo $fieldset['id']; ?>">
							<?php if( !empty($fieldset['fields']) && is_array($fieldset['fields']) ) : ?>
										...
										<td>
											...
											<?php if ( isset($field_settings[$field_id]['custom_row']) ) : ?>
												<ul>
													...
													<li><strong><?php _e('Enabled', \jcf\JustCustomFields::TEXTDOMAIN); ?></strong>: <?php if($enabled) _e('Yes', \jcf\JustCustomFields::TEXTDOMAIN); else  _e('No', \jcf\JustCustomFields::TEXTDOMAIN);?></li>
												</ul>
											<?php endif; ?>
										</td>
										<?php if ( !isset($field_settings[$field_id]['custom_row']) ) : ?>
								...
								<?php endforeach; ?>
							<?php else : ?>
							<tr><td colspan="4" align="center"><?php _e('Please create fields for this fieldset', \jcf\JustCustomFields::TEXTDOMAIN); ?></td></tr>
							<?php endif; ?>
						</tbody>
					</table>
				</div>
		...
		<div class="jcf_columns jcf_width50p">
			<div class="jcf_inner_box">
				<h3 class="header"><?php _e('Add Fieldset', \jcf\JustCustomFields::TEXTDOMAIN); ?></h3>
				<div class="jcf_inner_content">
					<form action="#" id="jcform_add_fieldset" method="post" class="jcf_form_horiz">
						<fieldset>
							<label for="jcf_fieldset_title"><?php _e('Title:', \jcf\JustCustomFields::TEXTDOMAIN); ?> </label>
							<input type="text" class="text" name="jcf_fieldset_title" id="jcf_fieldset_title" value="" />
							<input type="submit" name="jcf_add_fieldset" value="<?php _e('Add', \jcf\JustCustomFields::TEXTDOMAIN); ?>" />
							<?php echo jcf_print_loader_img(); ?>
						</fieldset>
					</form>
				</div>
