<div class="wrap">
	...
	<h2><?php _e('Just Custom Fields Export', \jcf\JustCustomFields::TEXTDOMAIN); ?></h2>
		...
		<?php if ( $post_types ): ?>
			<form method="post" id="jcf_export_fields" action="<?php echo get_home_url(); ?>/wp-admin/admin-ajax.php" >
				...
				<div id="jcf_save_export_fields_content">
					<p><?php _e('You should choose Fields to export:', \jcf\JustCustomFields::TEXTDOMAIN); ?></p>
													...
													<tbody id="the-list-<?php echo $fieldset_id; ?>">
														<?php if ( !empty($fieldset['fields']) ) : ?>
															<?php foreach ( $fieldset['fields'] as $field_id => $field ) : ?>
																...
																<td><?php if ( $field_settings[$key][$field_id]['enabled'] ) _e('Yes', \jcf\JustCustomFields::TEXTDOMAIN);
										   else _e('No', \jcf\JustCustomFields::TEXTDOMAIN); ?></td>
						<?php else: ?>
																<td>
																	<ul>
																		...
																		<li><strong><?php _e('Slug', \jcf\JustCustomFields::TEXTDOMAIN); ?></strong>: <em><?php echo $field_settings[$key][$field_id]['slug']; ?></em></li>
																		<li><strong><?php _e('Enabled', \jcf\JustCustomFields::TEXTDOMAIN); ?></strong>: <em><?php if ( $field_settings[$key][$field_id]['enabled'] ) _e('Yes', \jcf\JustCustomFields::TEXTDOMAIN);
							else _e('No', \jcf\JustCustomFields::TEXTDOMAIN); ?></em></li>
																	</ul>
																</td>
																<td colspan="3" >
							...
							<?php if ( !empty($field_setting) && is_array($field_setting) ): ?>
								<?php foreach ( $field_setting as $collection_field_id => $collection_field_value ): ?>
									...
									<?php endforeach; ?>
																					...
																					<td><?php if ( $collection_field_value['enabled'] ) _e('Yes', \jcf\JustCustomFields::TEXTDOMAIN);
									else _e('No', \jcf\JustCustomFields::TEXTDOMAIN); ?></td>
	...
	<?php endforeach; // foreach post types  ?>
				...
				<div class="jcf-modal-button">
					<input type="submit" class="button-primary" name="export_fields" value="<?php _e('Export', \jcf\JustCustomFields::TEXTDOMAIN); ?>" />
				</div>
			</form>
<?php endif; ?>
	</div>
